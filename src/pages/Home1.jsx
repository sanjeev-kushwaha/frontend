...
//   useEffect(() => {
//     axios
//       .get("http://localhost:8000/api/videos/videos")
...
//       {/* HEADER */}
//       <header className="flex flex-col sm:flex-row justify-between items-center px-6 py-4 bg-black border-b border-gray-700">
//         <h1 className="text-3xl sm:text-4xl font-bold text-orange-500">Spire▶️play</h1>
//         <nav className="flex flex-wrap justify-center gap-6 mt-3 sm:mt-0 text-sm sm:text-base text-gray-300">
//           {["Home", "TV Shows", "Movies", "Originals", "Recently Added"].map((item, i) => (
//             <a key={i} href={`#${item.toLowerCase().replace(/\s+/g, "")}`} className="hover:text-[#e50914] transition">
//               {item}
//             </a>
//           ))}
//         </nav>
//         <div className="flex items-center gap-4 text-lg mt-3 sm:mt-0">
//           <i className="fas fa-search cursor-pointer hover:text-[#e50914]"></i>
//           <i className="fas fa-bell cursor-pointer hover:text-[#e50914]"></i>
...
//             <img
//               src={`http://localhost:8000/api/videos/thumbnail/${videos[0]._id}`}
//               alt={videos[0].filename}
//               className="w-full h-full object-cover"
//             />
//             <div className="absolute inset-0 bg-gradient-to-r from-black via-black/70 to-transparent" />
//             <div className="absolute bottom-10 left-5 md:left-12 z-10">
//               <h2 className="text-3xl md:text-5xl font-bold">{videos[0].title}</h2>
//               <div className="flex items-center gap-2 text-yellow-400 mt-2 text-lg font-semibold">
//                 ★ ★ ★ ★ ☆ <span>5</span>
//                 <span className="bg-yellow-500 text-black px-2 rounded font-bold ml-2">IMDB</span>
//               </div>
//               <p className="mt-2 max-w-md text-sm md:text-base text-gray-300">
//                 {videos[0].description?.slice(0, 150)}...
//               </p>
//               <button className="mt-4 px-6 py-2 bg-white text-black rounded font-semibold hover:bg-red-700 transition">
//                 Watch Now
//               </button>
//             </div>
//           </Link>
...
//           Modern <span className="text-[#e50914]">Music</span> & Latest <span className="text-[#e50914]">Songs</span> 🎶
//         </h1>
...
//                 className="absolute left-0 top-1/2 z-10 -translate-y-1/2 bg-black/70 p-2 rounded-full hover:bg-black transition block"
//               >
//                 <ChevronLeft size={26} />
//               </button>
...
//                 className="flex overflow-x-auto no-scrollbar space-x-4 scroll-smooth pr-4"
//               >
//                 {videos.map((video) => (
//                   <div
//                     key={`${section}-${video._id}`}
//                     className="relative group flex-shrink-0 w-[45%] sm:w-[30%] md:w-[22%] lg:w-[15%]"
//                   >
//                     <Link to={`/watch/${video._id}`}>
//                       <img
//                         src={`http://localhost:8000/api/videos/thumbnail/${video._id}`}
//                         alt={video.filename}
//                         className="w-full h-40 sm:h-48 md:h-56 lg:h-64 object-cover rounded-md hover:scale-105 transition-transform duration-300"
//                       />
//                     </Link>

//                     {/* Play Now Button - shown only on hover */}
//                     <Link
//                       to={`/watch/${video._id}`}
//                       className="hidden group-hover:flex items-center justify-center absolute inset-0 bg-black/60 text-white font-semibold text-sm sm:text-base hover:bg-black/80 transition rounded-md"
//                     >
//                       ▶ Play Now
//                     </Link>
//                   </div>
...
//                 className="absolute right-0 top-1/2 z-10 -translate-y-1/2 bg-black/70 p-2 rounded-full hover:bg-black transition block"
//               >
...
//       {/* FOOTER */}
//       <footer className="px-6 py-10 border-t border-gray-700 text-sm text-gray-400">
...
//             "Privacy", "Legal Notices", "Corporate Information", "Contact Us",
//           ].map((item, idx) => (
    ...
    const fetchData = async () => {
      try {
        ...
        for (let i = 0; i < maxLength; i++) {
          if (videoRes.data[i]) interleaved.push({ type: "video", data: videoRes.data[i] });
          if (songRes.data[i]) interleaved.push({ type: "music", data: songRes.data[i] });
        }
  ...
  return (
    <div className="bg-[#323131] text-white font-sans min-h-screen overflow-hidden">
      ...
      <header className="sticky top-0 z-50 bg-[#323131] px-6 py-4 flex items-center justify-between border-b border-gray-800 backdrop-blur-md">
        {/* LOGO */}
        <Link to="/" className="text-2xl sm:text-3xl font-extrabold tracking-wide text-white hover:scale-105 transition">
          <span className="text-[#e50914]">LO</span><span className="text-white">GO</span>
        </Link>

        {/* NAVIGATION */}
        <nav className="hidden sm:flex gap-6 text-gray-300 text-sm sm:text-base">
          {["Home", "TV Shows", "Originals", "Recently Added"].map((item, i) => (
            <a
              key={i}
            ...
            >
              {bannerSlides[bannerIndex].type === "video" ? (
                <Link to={`/watch/${bannerSlides[bannerIndex].data._id}`}>
                  ...
                  <div className="absolute bottom-10 left-5 md:left-12 z-10">
                    <h2 className="text-3xl md:text-5xl font-bold">{bannerSlides[bannerIndex].data.originalName}</h2>
                    <div className="flex items-center gap-2 text-yellow-400 mt-2 text-lg font-semibold">
                      ★ ★ ★ ★ ☆ <span>5</span>
                      <span className="bg-yellow-500 text-black px-2 rounded font-bold ml-2">IMDB</span>
                    </div>
                ...
                <Link to="/music">
                  <img
                    src={`http://localhost:8000${bannerSlides[bannerIndex].data.thumbnail}`}
                    alt={bannerSlides[bannerIndex].data.title}
                    className="w-full h-full object-cover"
                  />
                  <div className="absolute inset-0 bg-gradient-to-r from-[#0b0c10] via-black/80 to-transparent" />
                  <div className="absolute bottom-10 left-5 md:left-12 z-10">
                    <h2 className="text-3xl md:text-5xl font-bold">{bannerSlides[bannerIndex].data.title}</h2>
                    <p className="text-lg text-gray-300 mt-2">{bannerSlides[bannerIndex].data.artist}</p>
                    <button className="mt-4 px-6 py-2 bg-[#e50914] text-white rounded font-semibold hover:bg-red-600 transition">
                      Listen Now
                    </button>
                  </div>
      ...
      <section className="text-center my-12">
        ...
        <div className="mt-6 flex justify-center">
          <Link
            to="/music"
            className="group relative w-full max-w-3xl overflow-hidden rounded-2xl shadow-xl transition-transform hover:scale-105"
          >
            <img
            ...
            >
                ...
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 backdrop-blur-sm bg-gradient-to-t from-black via-black/80 to-transparent transition-all duration-500 ease-out p-4 flex flex-col justify-end space-y-2">
                  ...
                  <div className="flex items-center text-white text-sm space-x-1 ">
                    <Clock size={16} />
                    <span className="pl-2">5:23</span> {/* Hardcoded duration */}
                  </div>
...
  })}
  ...
  < div className = "fixed bottom-0 left-0 right-0 z-50 bg-[#1c1c1e] sm:hidden border-t border-gray-700 flex justify-around items-center py-2 text-xs text-white" >
        ...
        <Link to="/category" className="flex flex-col items-center justify-center">
          <FaFolder className="text-lg mb-1" />
          Category
        </Link>
        <Link to="/watchlist" className="flex flex-col items-center justify-center">
